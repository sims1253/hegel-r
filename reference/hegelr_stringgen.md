...
String generators are immutable and shareable (ARCHITECTURE.md section
4), so they are created once per schema and cached in `hegelr_env` for
the process lifetime. `schema_key` is a deparse digest of the schema;
